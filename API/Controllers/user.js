...
export const register = async (req, res) => {
  ...
  try {
    ...
    if (user) return res.json({ message: "Use Already exist", success: false });
...
export const users = async(req,res)=>{
   try{
      let user = await User.find().sort({created:-1});
      res.json(user)
   }catch(error){
...
export const profile = async (req, res) => {
  res.json({ user: req.user }); // This will return the complete user object
};
